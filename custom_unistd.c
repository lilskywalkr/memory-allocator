...
void __attribute__((destructor)) memory_check(void)
{
    ...
    printf("\n### Stan plotkow przestrzeni sterty:\n");
    printf("    Plotek poczatku: [%s]\n", first == 0 ? "poprawny" : "USZKODZONY");
    printf("    Plotek konca...: [%s]\n", last == 0 ? "poprawny" : "USZKODZONY");

    printf("### Podsumowanie: \n");
    printf("    Calkowita przestrzeni pamieci....: %lu bajtow\n", mm.start_mmap - mm.start_brk);
    printf("    Pamiec zarezerwowana przez sbrk(): %lu bajtow\n", mm.brk - mm.start_brk);
